...
def train(trainData, trainLabel, kMode=0):
    ...
    '''
    if 0==kMode:
        clf = OneVsRestClassifier(LinearSVC(random_state=0))   # default kernel is 'rbf'
        clf.fit(trainData, trainLabel)
    elif 1 == kMode:
        clf = OneVsRestClassifier(estimator=svm.SVC(kernel='poly', degree=2))
        clf.fit(trainData, trainLabel)
    elif 2 == kMode:
        clf = OneVsRestClassifier(estimator=svm.SVC(C=10, random_state=0))
        clf.fit(trainData, trainLabel)
    elif 3 == kMode:
        clf = OneVsRestClassifier(estimator=svm.SVC(kernel='sigmoid'))
        clf.fit(trainData, trainLabel)
    else:
